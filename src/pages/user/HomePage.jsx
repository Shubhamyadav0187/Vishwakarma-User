...
const HomePage = () => {
  const sections = useMemo(() => [
    { title: "Tiles", data: PRODUCT_DB.Tiles },
    { title: "Steel", data: PRODUCT_DB.Steel },
    { title: "Iron", data: PRODUCT_DB.Iron },
    { title: "Aluminium", data: PRODUCT_DB.Aluminium },
    { title: "Cement", data: PRODUCT_DB.Cement },
    { title: "Paints", data: PRODUCT_DB.Paints },
    { title: "Plumbing", data: PRODUCT_DB.Plumbing },
  ], []);

  return (
    <main className="bg-gradient-to-r from-[#0B0C10] to-blue-900 text-[#EB5757]">
      <Hero />
      {sections.map((s) => (
        <CategorySlider key={s.title} title={s.title} products={s.data} />
      ))}
    </main>
  );
};
